...
def generate_report(df):
    ...
    with open(report_file, 'w') as f:
        ...
        f.write(f"Highest temperature: {df['temperature'].max()}Â°F in {df.loc[df['temperature'].idxmax(), 'city']}\n")

        f.write(f"Lowest temperature: {df['temperature'].min()}Â°F in {df.loc[df['temperature'].idxmin(), 'city']}\n")

        f.write(f"Average temperature across cities: {df['temperature'].mean():.1f}Â°F\n\n")
        ...
        for _, row in df.iterrows():
            ...
            f.write(f"  Temperature: {row['temperature']}Â°F (Feels like: {row['feels_like']}Â°F)\n")
